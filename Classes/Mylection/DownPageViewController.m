...
//    fontValue = paramssetting.fontSizeIndex;
...
- (void)motionEnded:(UIEventSubtype)motion withEvent:(UIEvent *)event
{
    if (motion == UIEventSubtypeMotionShake)
    {
        ...
        SettingData * paramssetting = [SettingData sharedSettingData];//白天
        [paramssetting updateFontColorId:1];
        [paramssetting updateBackgroundFile:1];
        [paramssetting updateBackgroundAlpha: 0];
        ...
        SettingData * paramssetting = [SettingData sharedSettingData];//夜晚
        [paramssetting updateFontColorId:2];
        [paramssetting updateBackgroundFile:1];
        [paramssetting updateBackgroundAlpha: 1];
...
- (void)viewDidLoad {
	...
	self.m_readView = [[ReadView alloc] initWithFrame: CGRectMake(0,0,320, DEVICE_HEIGHT-44-49-20)];
...
- (void)twoFingerPinch:(UIPinchGestureRecognizer *)recognizer
{
 ...
 //  NSLog(@"Pinch scale: %f_____Pinch velocity: %f", recognizer.scale,recognizer.velocity);
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer{
    ...
    NSLog(@"Pinch scale: %f_____Pinch velocity: %f",  pinch.scale,pinch.velocity);
    
    if (pinch.velocity>0) {
        
        if (fontValue==0) {
            [paramssetting updateFontSize:1];
        }
        if (fontValue==1) {
              [paramssetting updateFontSize:2];
        ...
        if (fontValue==1) {
             [paramssetting updateFontSize:0];
